...
public class ConnectAckHeaderTest {
    ...
    @Test
    public void readConnectAckHeaderTest() throws IOException, LimeBrokerException {
        ByteBuf buf = Unpooled.buffer();
        try {
            buf.writeByte(0b00000000);
            ConnectAckHeader cah = ConnectAckHeader.readConnectAckHeader(buf);
            assertEquals(ConnectReturnCode.CONNECTION_ACCEPTED, cah.getConnectReturnCode());
        } finally {
            buf.release();
        }
    }
