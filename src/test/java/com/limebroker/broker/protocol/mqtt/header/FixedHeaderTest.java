...
public class FixedHeaderTest
{
    private Map<Long, byte[]> generateRequiredLengthTestData()
    {
        ...
        m.put(Long.valueOf(16384), new byte[] {(byte) 0x80, (byte) 0x80, 0x01});
        m.put(Long.valueOf(2097152), new byte[] {(byte) 0x80, (byte) 0x80, (byte) 0x80, 0x01});
        m.put(Long.valueOf(FixedHeader.MAX_LENGTH), new byte[] {(byte) 0xFF, (byte) 0xFF, (byte) 0xFF, 0x7F});
        return m;
    }
    ...
    public void encodeRemainingLengthTest()
    {
        try
        {
            for (Map.Entry<Long, byte[]> entry : generateRequiredLengthTestData().entrySet())
            {
                ByteBuffer buffer = ByteBuffer.allocate(entry.getValue().length);
                FixedHeader.encodeRemainingLength(entry.getKey().longValue(), buffer);
